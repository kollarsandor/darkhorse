...
pub const AccessRight = enum(u8) {
    ...
    pub fn fromBitmask(mask: u8) Self {
        if (mask >= 16) return .ADMIN;
        if (mask >= 8) return .DELETE;
        if (mask >= 4) return .EXECUTE;
        if (mask >= 2) return .WRITE;
        if (mask >= 1) return .READ;
        return .NONE;
    }
};
...
pub const SecureObject = struct {
    ...
    pub fn init(allocator: Allocator, id: u64, name: []const u8, label: SecurityLabel, owner_id: u64) !*Self {
        ...
        obj.* = Self{
            ...
            .owner_id = owner_id,
            .data_hash = std.mem.zeroes([32]u8),
            .allocator = allocator,
        };
        return obj;
    }
    ...
    pub fn verifyDataHash(self: *const Self, data: []const u8) bool {
        ...
        return timingSafeEql([32]u8, self.data_hash, computed);
    }
};
...
pub const BisimulationRelation = struct {
    ...
    fn computeStateHash(state: *const SystemState) [32]u8 {
        var hasher = Sha256.init(.{});
...
pub const HashChainBlock = struct {
    ...
    pub fn init(allocator: Allocator, index: u64, data: []const u8, previous_hash: [32]u8) !*Self {
    ...
    pub fn verify(self: *const Self) bool {
        var computed_data_hash: [32]u8 = undefined;
        Sha256.hash(self.data, &computed_data_hash, .{});
        if (!timingSafeEql([32]u8, self.data_hash, computed_data_hash)) return false;
        ...
        var computed_block_hash: [32]u8 = undefined;
        block_hasher.final(&computed_block_hash);

        return timingSafeEql([32]u8, self.block_hash, computed_block_hash);
    }
};
...
pub const HashChain = struct {
    ...
    pub fn init(allocator: Allocator) !Self {
        ...
        const genesis_block = try HashChainBlock.init(allocator, 0, "GENESIS", std.mem.zeroes([32]u8));
    ...
    pub fn verify(self: *const Self) bool {
        ...
        while (i < self.blocks.items.len) : (i += 1) {
            ...
            if (i > 0) {
                ...
                if (!timingSafeEql([32]u8, block.previous_hash, prev_block.block_hash)) {
                    return false;
                }
            }
        }
    ...
    pub fn latestHash(self: *const Self) [32]u8 {
...
pub const MerkleProof = struct {
    leaf_hash: [32]u8,
    sibling_hashes: ArrayList([32]u8),
    ...
    pub fn init(allocator: Allocator) Self {
        return Self{
            .leaf_hash = std.mem.zeroes([32]u8),
            .sibling_hashes = ArrayList([32]u8).init(allocator),
            .directions = ArrayList(bool).init(allocator),
            .root_hash = std.mem.zeroes([32]u8),
            .allocator = allocator,
        };
    }
    ...
    pub fn verify(self: *const Self, data: []const u8) bool {
        ...
        if (!timingSafeEql([32]u8, current_hash, self.leaf_hash)) return false;
        ...
        return timingSafeEql([32]u8, current_hash, self.root_hash);
    }
};
...
pub const MerkleTree = struct {
    ...
    pub fn getRootHash(self: *const Self) ?[32]u8 {
    ...
    fn verifyNode(self: *const Self, node_idx: usize) bool {
        ...
        if (node.right_idx) |right_idx| {
            ...
            if (!timingSafeEql([32]u8, node.hash, computed_hash)) return false;

            return self.verifyNode(left_idx) and self.verifyNode(right_idx);
        } else {
            if (!timingSafeEql([32]u8, node.hash, left.hash)) return false;
...
pub const Commitment = struct {
    ...
    pub fn reveal(self: *Self, value: []const u8) !bool {
        ...
        if (timingSafeEql([32]u8, self.commitment_hash, computed_hash)) {
    ...
    pub fn verifyBinding(self: *const Self, claimed_value: []const u8) bool {
        ...
        var computed_hash: [32]u8 = undefined;
        hasher.final(&computed_hash);
        return timingSafeEql([32]u8, self.commitment_hash, computed_hash);
    }
};
...
pub const CommitmentScheme = struct {
    ...
    pub fn getCommitmentHash(self: *const Self, id: u64) ?[32]u8 {
...
pub const CryptographicProof = struct {
    ...
    pub fn init(allocator: Allocator, proof_id: u64, proof_type: ProofType, statement: []const u8, witness: []const u8) !*Self {
        ...
        var witness_hash: [32]u8 = undefined;
    ...
    pub fn verify(self: *const Self, witness: []const u8) bool {
        var computed_witness_hash: [32]u8 = undefined;
        Sha256.hash(witness, &computed_witness_hash, .{});

        if (!timingSafeEql([32]u8, self.witness_hash, computed_witness_hash)) return false;
    ...
    pub fn getProofHash(self: *const Self) [32]u8 {
        var hasher = Sha256.init(.{});
...
pub const IntegrityVerifier = struct {
    ...
    data_hashes: AutoHashMap(u64, [32]u8),
    next_id: u64,
    allocator: Allocator,
    ...
    pub fn init(allocator: Allocator) !Self {
        return Self{
            ...
            .data_hashes = AutoHashMap(u64, [32]u8).init(allocator),
            .next_id = 1,
            .allocator = allocator,
        };
    }
    ...
    pub fn verifyIntegrity(self: *const Self, data_id: u64, data: []const u8) bool {
        ...
        return timingSafeEql([32]u8, expected_hash, computed_hash);
    }
    ...
    pub fn getMerkleRoot(self: *const Self, tree_id: u64) ?[32]u8 {
    ...
    pub fn proveIntegrity(self: *const Self, data_id: u64, data: []const u8, allocator: Allocator) !*CryptographicProof {
        ...
        if (!timingSafeEql([32]u8, expected_hash, computed_hash)) {
...
pub const SecurityProof = struct {
    ...
    pub fn verify(self: *const Self) bool {
        ...
        var computed_binding: [32]u8 = undefined;
